...
%% Mod start here.
% 1) select files create new head model for all subjects
% 2) replace data from one head model in teh other files.
...
rng(42) % please comment this line to make a real random permutation
...
for iSubject = 1 : nSubjects
    ...
    %% NOTA GIORGIO modifica qui selezione con tag per selezionare sorgenti corrette.
...
for iSubject = 1 : nSubjects
    ...
    for iSegment = 1 : size(timeIni, 2)
        ...
        %% GIORGIO modify here for sensors
...
sPsdMat = in_bst_timefreq(sPsdFiles(1,1).FileName, 0, 'RowNames', 'Freqs'); %% GIORGIO modify rownames
Freqs = sPsdMat.Freqs;
...
iccFreqsScouts = reshape(icc, [nFreqs, nScouts]); %GIORGIO change nScouts
...
% Cortex surface display for band ICC values %% GIORGIO REMOVE EVERYTHING OR REPLACE WITH Channel visualizationp
