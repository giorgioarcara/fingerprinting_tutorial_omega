...
for iSubj=1:nSubjects
    ...
    HeadModelFile = bst_get('HeadModelForStudy',   RawFileHead(1).iStudy);
    HeadModel=in_bst_headmodel(HeadModelFile.FileName);


    HeadModelFileSame = bst_get('HeadModelForStudy',   RawFileHeadSame(1).iStudy);
...
for iSubject = 1 : nSubjects
    ...
    halfTime = sData.Time(2)/2; % Note GA: fixed to work with Imported data
    % First segment
