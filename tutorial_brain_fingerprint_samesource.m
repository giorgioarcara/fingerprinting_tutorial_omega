function tutorial_brain_fingerprint_samesource(ProtocolNameOmega, reports_dir)
...
%% == !!! WARNINGS !! ==
% Note 1: this script is meant to be launched after tutorial_omega.m script
% Launching this script in a protocol where multiple head model or source 
% kernels are available could lead to wrong results.

% Note 2: this script create an object with a very large size (about 13
% GB). Check you have enough space before running it.
%
% Note 3: as data are resampled also empty room noise data should have been
% resampled for noise covariance computation. This is not done here for
% sake of simplicity and as it is not expected to affect the results.

%% ===== CREATE NEW CHANNEL DATA FROM SAME SOURCE =====
% set Subjects

SubjectNames = {'sub-0002', 'sub-0003', 'sub-0004', 'sub-0006', 'sub-0007'};
nSubjects = length(SubjectNames);

% select the index of the Subject that will be the reference for all other
% Subjects.
refSubject = 1

% The script back projects source data from a reference subject to the
% channels of other subjects using the head model of the destination Subject
% as a transformation matrix.

% the script perform the following steps.

% PART 1
% 1) resample at 200 Hz (to reduce computational burden) and import the
% file. (delete intermediate step).
% 2) copy existing noise covariance and existing head model to the Study 
% included the Imported Raw Data

% PART 2
% for ref file
% 3) extract source (this will take quite a lot of time).

% PART 3
% for all data
% 1) select correctly imported Raw Data and load in workspace
% 2) load the head model in workspace.
% 3) load the same source data
% 4) multiply the head model with the source data to get new data.
% 5) Substitute in the imported Raw Data and save.
% 6) Launch the rest of brain_fingeprint tutorials on these new data.
...
for iSubj=1:nSubjects
    ...
    % get only first study if more are available
    RawFile = RawFile(1);

    % Process: Resample: 200Hz
    ResFile = bst_process('CallProcess', 'process_resample', RawFile, [], ...
        'freq',     200, ...
        'read_all', 0);

    % Process: Import MEG/EEG: Time
    ImpResFile = bst_process('CallProcess', 'process_import_data_time', ResFile, [], ...
        'subjectname',   SubjectNames{iSubj}, ...
        'condition',     '', ...
        'timewindow',    [0, 549.999], ...
        'split',         0, ...
        'ignoreshort',   1, ...
        'usectfcomp',    1, ...
        'usessp',        1, ...
    ...
    ResFile = bst_process('CallProcess', 'process_delete', ResFile, [], ...
        'target', 2);  % Delete selected folders

    % copy head model and noise covariance (the active head model will be
    % selected)
    HeadModelFile = bst_get('HeadModelForStudy', RawFile.iStudy);

    db_set_headmodel(file_fullpath(HeadModelFile.FileName), 'AllConditions');
    db_set_noisecov(RawFile.iStudy, 'AllConditions');

    % Process: Compute sources [2018]
    SourceFile = bst_process('CallProcess', 'process_inverse_2018', ImpResFile, [], ...
        'output',  2, ...  % Kernel only: one per file
        'inverse', struct(...
        'Comment',        'dSPM: MEG (New)', ...
...
% Get reference source file and extract it
RefSourceFile = bst_process('CallProcess', 'process_select_files_results', [], [], ...
    'subjectname',   SubjectNames{refSubject}, ...
    'condition',     '', ...
    'tag',           '', ...
    'includebad',    0, ...
    'includeintra',  0, ...
...
% select only the reference source of resampled data
SourceNames = {RefSourceFile.FileName};
SelString=regexpi(SourceNames, 'resample'); %
SelString_ind=find(~cellfun(@isempty, SelString));

RefSourceFile = RefSourceFile(SelString_ind);

% Process: Extract values: [all]
RefSourceObj = bst_process('CallProcess', 'process_extract_values', RefSourceFile, [], ...
    'timewindow', [], ...
    'scoutsel',   {}, ...
    'scoutfunc',  1, ...  % Mean
    'isnorm',     0, ...
...
for iSubj=1:nSubjects
    ...
    % get Ref source data
    source = in_bst_data(RefSourceObj.FileName);


    % run the code only if the subject is different from the Reference
    % subject
    if iSubj~=refSubject

        % load on Matlab source data
        SourceObj = in_bst_data(RefSourceObj.FileName);
        source = SourceObj.ImageGridAmp;
        ...
        % select only the reference file of resampled data
        StudiesInd = {RawFile.iStudy};
        RawNames = {RawFile.FileName};
        SelString=regexpi(RawNames, 'resample'); %
        SelString_ind=find(~cellfun(@isempty, SelString));

        SelRawFile = RawNames(SelString_ind);
        SelRawStudy= StudiesInd(SelString_ind);


        HeadModelFile = bst_get('HeadModelForStudy',   SelRawStudy{1});
        HeadModel=in_bst_headmodel(HeadModelFile.FileName);

        RawData = in_bst_data(file_fullpath(SelRawFile{1}));
        ...
        RawData.Notes = ['Replaced Data with Source projected data from ' SubjectNames{refSubject}];

        bst_save(file_fullpath(SelRawFile{1}), RawData);
...
for iSubject = 1 : length(SubjectNames)
   ...
   sRawDataFiles = [sRawDataFiles, SelRawFile];
    ...
    % (Note: to select the correct source I use the comment here.)
...
sSimilarityMat.Comment      = 'Similarity matrix samesource';
...
sDiffMat.Comment = 'Differentiability samesource';
sDiffMat.Time     = [0, 1];
...
for iBand = 1 : nBands
    ...
    sIccBandMat.Comment = ['ICC_' BandNames{iBand}, '_samesource'];
...
for iBand = 1 : nBands
    ...
    sIccBandMat.Comment = ['ICC_' BandNames{iBand} '_samesource']; %added same head comment
    sIccBandMat.Time    = [0, 1];
