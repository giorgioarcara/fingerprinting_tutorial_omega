%% RECOMPUTE MEASURES FOR RANDOMHEAD
...
% Subjects
% Frequency range
LowerFreq =   0.5; % Hz, inclusive
UpperFreq = 50; % 150; % Hz, exclusive
...
BandLowerFreqs = [ 0.5, 4, 8, 13, 30]; % Hz, inclusive
BandUpperFreqs = [ 4, 8, 13, 30, 48]; % Hz, exclusive
...
for iSubject = 1 : nSubjects
    ...
    % ADDED SELECTION HERE:
    % exclude files with "resample" (imported files for samesource
    % condition)
    RawNames = {sRawDataFiles.FileName};
    SelString=regexpi(RawNames, 'resample'); %
    SelString_ind=find(cellfun(@isempty, SelString)); % exclude files that include "resample" in the Filename
    sRawDataFiles = sRawDataFiles(SelString_ind);
...
sSimilarityMat.Comment      = ['Similarity matrix randomhead',' ', tag];
...
sDiffMat.Comment = ['Differentiability randomhead', tag];
sDiffMat.Time     = [0, 1];
...
for iBand = 1 : nBands
    ...
    sIccBandMat.Comment = ['ICC_' BandNames{iBand}, '_randomhead','_', tag];
...
for iBand = 1 : nBands
    ...
    sIccBandMat.Comment = ['ICC_' BandNames{iBand} '_randomhead', '_', tag]; %added random head comment
    sIccBandMat.Time    = [0, 1];
